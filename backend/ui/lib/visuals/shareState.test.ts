...
test("a filtered page reads as something a person could have typed", () => {
	...
	assert.equal(
		written,
		"page=detailByRegion&businessUnit=Hardware,Software&orderDate=2026-01-01..2026-03-31&by=productLine",
	);
});
...
test("a hand typed link works", () => {
	...
	const back = decodeShareParams(
		new URLSearchParams("businessUnit=Hardware&orderDate=2026-06-01.."),
		context,
	);
	assert.deepEqual(back?.filters?.["w-unit"], [
		{ field: "Business Unit", op: "eq", values: ["Hardware"] },
	]);
