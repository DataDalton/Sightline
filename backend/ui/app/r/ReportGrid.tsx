...
import { fillsHeight } from "../../lib/visuals/catalog";
import {
	fillToViewport,
	gridGap,
	heightForRows,
...
export function ReportGrid({
	...
	const placed = useMemo(() => {
		...
		// at the origin.
		let fallbackY = 0;
		const items = visuals.map((visual) => {
			...
			const sized: Rect = live
				...
				: override
					? { ...rect, w: override.w ?? rect.w, h: override.h ?? rect.h }
					: rect;
	...
	}, [visuals, narrow, sizeFor, available, state]);
	...
	const rows = placed.reduce((max, i) => Math.max(max, i.rect.y + i.rect.h), 0);
			...
			{placed.map((item) => {
				...
				return (
					...
					>
						...
						{!narrow && (
							<>
								{(["resize-e", "resize-s", "resize-se"] as const).map(
									(kind) => (
										<div
											key={kind}
