...
export async function register() {
	...
	const { initPlatformSchema, sweepExpired } = await import(
		"@/lib/platform/schema"
	);
	const { loadSettings, startSettingsPolling } = await import("@/lib/settings");
	const { loadRegistry, startRegistryPolling } = await import(
		"@/lib/semantic/registry"
	);
	...
	try {
		...
		if (isDatabricksApp) throw error;
